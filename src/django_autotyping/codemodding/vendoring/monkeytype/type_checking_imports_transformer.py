...
class MoveImportsToTypeCheckingBlockVisitor(ContextAwareTransformer):
    ...
    def store_imports_in_context(
        ...
        context.scratch[MoveImportsToTypeCheckingBlockVisitor.CONTEXT_KEY] = (import_items_to_be_moved,)
    ...
    @staticmethod
    def _replace_pass_with_imports(placeholder_module: Module, import_module: Module) -> Module:
    ...
    def transform_module_impl(
        ...
        context_contents = self.context.scratch.get(MoveImportsToTypeCheckingBlockVisitor.CONTEXT_KEY)
        if context_contents is not None:
            ...
            self.import_items_to_be_moved = self._remove_typing_module(self.import_items_to_be_moved)
...
class RemoveImportsTransformer(CSTTransformer):
    ...
    def leave_ImportFrom(
        ...
        for name in updated_node.names:
            ...
            found = False
            for import_item in self.import_items_to_be_removed:
                if import_item.module_name == module_name and import_item.obj_name == name_value:
                    found = True
                    break
            if not found:
                names_to_keep.append(name.with_changes(comma=MaybeSentinel.DEFAULT))
